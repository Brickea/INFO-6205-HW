 ...
 */
public class UF_HWQUPC_ALTER implements UF {
    /**
     ...
     */
    public UF_HWQUPC_ALTER(int n, boolean pathCompression) {
        count = n;
        parent = new int[n];
        height = new int[n];
        for (int i = 0; i < n; i++) {
    ...
    public int find(int p) {
        ...
        if (this.pathCompression) {
            // two loops, so that all intermediate nodes point to the root
            int temp = root;
            while (this.parent[temp] != temp) {
                temp = this.parent[temp];
            }
            this.parent[root] = temp;
        } else {
    ...
    private void mergeComponents(int i, int j) {
        ...
        if (this.height[rootI] >= this.height[rootJ]) {
            this.parent[rootJ] = this.parent[rootI];
            this.height[rootI] += this.height[rootJ];
        } else {
            this.parent[rootI] = this.parent[rootJ];
            this.height[rootJ] += this.height[rootI];
        }
    }
